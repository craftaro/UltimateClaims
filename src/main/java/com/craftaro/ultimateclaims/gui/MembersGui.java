...
public class MembersGui extends CustomizableGui {
    ...
    public MembersGui(UltimateClaims plugin, Claim claim) {
        ...
        this.plugin = plugin;
        this.setRows(6);
        this.setTitle(plugin.getLocale().getMessage("interface.members.title").getMessage().toString());
    ...
    private void showPage() {
        ...
        this.setItem("stats", 4, GuiUtils.updateItem(this.getItem(4),
                this.plugin.getLocale().getMessage("interface.members.statstitle").getMessage().toString(),
                this.plugin.getLocale().getMessage("interface.members.statslore")
                        ...
                        .processPlaceholder("members", this.claim.getMembers().size()).getMessage().toString().split("\\|")));

        // Filters
        this.setItem("type", 3, GuiUtils.updateItem(this.getItem(3),
                this.plugin.getLocale().getMessage("interface.members.changetypetitle").getMessage().toString(),
                this.plugin.getLocale().getMessage("general.interface.current")
                        .processPlaceholder("current",
                                this.displayedRole == ClaimRole.OWNER ? this.plugin.getLocale().getMessage("interface.role.all").getMessage().toString() : this.plugin.getLocale().getMessage(this.displayedRole.getLocalePath()).getMessage().toString())
                        .getMessage().toString().split("\\|")));
        this.setItem("sort", 5, GuiUtils.updateItem(this.getItem(5),
                this.plugin.getLocale().getMessage("interface.members.changesorttitle").getMessage().toString(),
                this.plugin.getLocale().getMessage("general.interface.current")
                        .processPlaceholder("current",
                                this.plugin.getLocale().getMessage(this.sortType.getLocalePath()).getMessage().toString())
                        .getMessage().toString().split("\\|")));
                ...
                this.setItem(row, col, GuiUtils.createButtonItem(ItemUtils.getPlayerSkull(skullPlayer),
                        ...
                        this.plugin.getLocale().getMessage("interface.members.skulllore")
                                .processPlaceholder("role",
                                        this.plugin.getLocale().getMessage(toDisplay.get(currentMember).getRole().getLocalePath()).getMessage().toString())
