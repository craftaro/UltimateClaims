...
public class AuditGui extends CustomizableGui {
    ...
    public AuditGui(UltimateClaims plugin, Claim claim, Player player) {
        ...
        this.player = player;
        this.setRows(6);
        this.setTitle(plugin.getLocale().getMessage("interface.audits.title").getMessage().toString());
        this.showPage();
    }
    ...
    private void showPage() {
        ...
        this.plugin.getAuditManager().getAudits(this.claim, audits -> {
            ...
            setNextPage(5, 7, GuiUtils.createButtonItem(XMaterial.ARROW, this.plugin.getLocale().getMessage("general.interface.next").getMessage().toString()));
            setPrevPage(5, 1, GuiUtils.createButtonItem(XMaterial.ARROW, this.plugin.getLocale().getMessage("general.interface.previous").getMessage().toString()));
            setOnPage(event -> showPage());
            setButton("exit", 8, GuiUtils.createButtonItem(XMaterial.OAK_DOOR, this.plugin.getLocale().getMessage("general.interface.exit").getMessage().toString()), (event) -> this.player.closeInventory());
