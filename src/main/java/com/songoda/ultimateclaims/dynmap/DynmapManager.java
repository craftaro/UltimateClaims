...
public class DynmapManager {
        ...
        if (corners != null) {
            ...
            for (AreaMarker aMarker : markerSet.getAreaMarkers()) {
                if (aMarker.getMarkerID().startsWith(claim.getId() + ":")) {
                    ...
                    for (ClaimCorners c : corners) {
                        if (aMarker.getMarkerID().equals(claim.getId() + ":" + c.chunkID)) {
                            remove = false;
                            break;
                        }
                    }
            ...
            for (ClaimCorners c : corners) {
                if (markerSet.findAreaMarker(claim.getId() + ":" + c.chunkID) == null) {
                    markerSet.createAreaMarker(claim.getId() + ":" + c.chunkID, "Claim #" + claim.getId(), false, claim.getFirstClaimedChunk().getWorld(), c.x, c.z, false);
                }
    ...
    public void refreshDescription(Claim claim) {
        ...
        String markerDesc = Settings.DYNMAP_BUBBLE.getString()
                ...
                .replace("${PowerLeft}", Methods.makeReadable(claim.getPowerCell().getTotalPower() * 60 * 1000));
    ...
    public void reload() {
        ...
        if (Settings.DYNMAP_ENABLED.getBoolean()) {
            this.markerSet = dynmapAPI.getMarkerAPI().createMarkerSet("UltimateClaims.chunks", Settings.DYNMAP_LABEL.getString(), dynmapAPI.getMarkerAPI().getMarkerIcons(), false);
            ...
            if (taskID == -1) {
                ...
                if (updateInterval > 0) {
                    taskID = Bukkit.getScheduler().scheduleSyncRepeatingTask(this.plugin, task, 20 * updateInterval, 20 * updateInterval);
                }
            }
        } else {
            if (taskID != -1) {
